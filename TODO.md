...
- **Phase 3 ⏳（保留・ユーザー判断「今は release までで止める」2026-07-19）**:
  実機 e2e。再開レシピ:
  1. slave（n9htqcr6g0-herdr 等）で `herdr-drover update`（→v0.5.5。現在 v0.4.4）。
...
### D. ↗窓 の agent 状態同期 — プロトタイプ完成（branch `feat/inj-agent-status`・未 merge/未 release）

リモートの agent_status を注入 pane に `pane.report_agent` で転記し、↗窓 を herdr に
「agent」として検出させる（tab/workspace の agent_status・`agent list/wait/focus` が
リモートに効く＝振り分け時に pane を read せず status で判定可能に）。**opt-in**＝
`DROVER_MIRROR_AGENTS`(env) / `mirror_agents`(config.json)・既定 OFF。producer は既に
agent_status/window_name を同期済み＝producer 改修ゼロ。

- 実装（2 コミット）: herdrapi `ReportAgent`/`ReleaseAgent`、reconcile `injAPIState`＋
  `mirrorInjectedAgent`（CREATE 後・既存の両経路で転記／agent 終了で release／CLOSE で
  追跡除去）、config ゲート（`reported==nil`＝無効）。実 herdr 隔離テスト＋config テスト＋
  disabled テスト緑・-race 緑・旧コード FAIL 確認済。
- herdr 実測トラップ（コードにコメント）: **seq は送らない**（per-source 単調増加ゲートで
  後続 report が無視される）／agent_status は表示5値・report の --state は4値、`done/idle→idle`
  へ写像（`pane_agent_status(state, seen)` で内部 Idle は未 seen→done）。
- 実機（mac-studio・config `mirror_agents:true`・稼働 daemon 差替）で 9/9 の ↗窓 が
  unknown→実状態、working↔idle が実 pane 状態と一致を確認。
- **残**: merge（main へ）＋ release 判断＋ `--help`/CLAUDE.md へ env var 追記。稼働 daemon は
  現在この branch の dirty ビルド（`v0.5.5-1-g…-dirty`）。
...
2. **↗窓 agent 状態同期**（上記 D・branch `feat/inj-agent-status` 完成＝main へ merge /
   release 判断待ち。稼働 daemon は現在この branch の dirty ビルド）。
...
- 稼働 launchd `com.4noha.herdr-drover`（pc=`mac-studio-herdr`・v0.5.5・
