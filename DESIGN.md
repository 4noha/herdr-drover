...
- **二段構え（不変条件: pane を消したまま終わらせない）**: herdr の
  `agent_session` が指す uuid は「復元可能な会話」を**保証しない**。claude は
  **最初のメッセージを送るまで jsonl を書かない**ので、起動しただけで未使用の
  セッションは「uuid はあるが `~/.claude/projects/**/<uuid>.jsonl` は無い」状態に
  なる（実測 2026-07-25・2 回発生）＝稀な破損ではなく**未使用セッションの通常状態**。`claude --resume <無い uuid>` は即 exit し、**単独 pane の Tab は
...
（クラウド共有 lib は外部 module github.com/4noha/drover-cloud へ切り出し済＝
 state/relayclient/selfupdate は import 依存。以前は cm からのバイト同一コピー
 を internal/cloud/state 等に置いていた＝重複解消。go.mod は replace で
 ../drover-cloud をローカル解決）
...
- 160×500 既定 viewport の padding/初回 full frame → 実ブラウザゲートで判定
  ...
  - **暫定既定値（実測根拠・ゲート未通過）**: bridge の clamp knob
    （MaxRows/MaxCols）の既定は 120×320。根拠は隔離 herdr 0.7.4 実測＝
    observe full frame は空白セルも明示塗りの約 20B/セル×cols×rows で、
    160×500 は raw 1.6MB/base64 2.1MB（帯域の純損失。pane 実 PTY 超過分は
    上寄せ空白 padding で scrollback 非含有）。cm Web term.js は接続時に
    500 行級×160 の固定 RESIZE を送るため必ずこの clamp を踏む＝bridge は
    丸め発生時に「要求→実サイズ」を必ずログする（沈黙丸め禁止）。**この
    既定値の最終判定（500 行 xterm 中の 120 行 render が使い物になるか）は
    Phase 2 実 Chrome display-oracle ゲートの判定事項として残っている**＝
    ゲート実施時に clamp ログと突き合わせて既定値を確定すること。
